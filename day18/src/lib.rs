...
fn simplify_and_evalute(expression: &String, start: usize, end: usize, subexpressions: &HashMap::<usize, Subexpression>, evaluator: &dyn Fn(String) -> usize) -> usize {
    let mut i = start;
...
fn evaluate_expression(expression: String, evaluator: &dyn Fn (String) -> usize) -> usize {
        ...
        match c {
            ...
            ')' => {
                ...
                subexpressions.insert(
                    open_pos,
                    Subexpression{
                        result: simplify_and_evalute(&expression, open_pos + 1, n - 1, &subexpressions, evaluator),
                        length: n - open_pos + 1,
                    }
                );
            },
    ...
    simplify_and_evalute(&expression, 0, expression.len() - 1, &subexpressions, evaluator)
}

pub fn part1(input: &Path) -> Result<(), Error> {
    let sum: usize = parse(input)?.take_while(|s| *s != "").map(|expression| evaluate_expression(expression, &evaluate_simple_expression)).sum();
...
pub fn part2(input: &Path) -> Result<(), Error> {
    let sum: usize = parse(input)?.take_while(|s| *s != "").map(|expression| evaluate_expression(expression, &evaluate_advanced_simple_expression)).sum();
...
fn test_evaluate_expression_simple() {
    ...
    assert_eq!(evaluate_expression(example, &evaluate_simple_expression), 204);

    let example = "2 * 3 + (4 * 5)".to_string();
    assert_eq!(evaluate_expression(example, &evaluate_simple_expression), 26);

    let example = "5 + (8 * 3 + 9 + 3 * 4 * 3)".to_string();
    assert_eq!(evaluate_expression(example, &evaluate_simple_expression), 437);

    let example = "5 * 9 * (7 * 3 * 3 + 9 * 3 + (8 + 6 * 4))".to_string();
    assert_eq!(evaluate_expression(example, &evaluate_simple_expression), 12240);
    
    let example = "((2 + 4 * 9) * (6 + 9 * 8 + 6) + 6) + 2 + 4 * 2".to_string();
    assert_eq!(evaluate_expression(example, &evaluate_simple_expression), 13632);
}

#[test]
fn test_evaluate_expression_advanced() {
    let example = "1 + (2 * 3) + (4 * (5 + 6))".to_string();
    assert_eq!(evaluate_expression(example, &evaluate_advanced_simple_expression), 51);

    
    let example = "2 * 3 + (4 * 5)".to_string();
    assert_eq!(evaluate_expression(example, &evaluate_advanced_simple_expression), 46);

    
    let example = "5 + (8 * 3 + 9 + 3 * 4 * 3)".to_string();
    assert_eq!(evaluate_expression(example, &evaluate_advanced_simple_expression), 1445);

    
    let example = "5 * 9 * (7 * 3 * 3 + 9 * 3 + (8 + 6 * 4))".to_string();
    assert_eq!(evaluate_expression(example, &evaluate_advanced_simple_expression), 669060);

    
    let example = "((2 + 4 * 9) * (6 + 9 * 8 + 6) + 6) + 2 + 4 * 2".to_string();
    assert_eq!(evaluate_expression(example, &evaluate_advanced_simple_expression), 23340);
}
